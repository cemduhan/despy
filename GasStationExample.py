...
def GasStation():   
    ...
    des.state.add_block(des.EnterBlock("GasStation", 5))  #1 Cars Enter gas station which has 5 gas pumps so sixth car must wait
    des.state.add_block(des.AdvanceBlock('Uniform',5,15))  #2 Filling the gas tank of a vehicle takes between 5 and 15 seconds
    des.state.add_block(des.EnterBlock("Cashier", 2))  #3 After filling gas tank car owner must enter the line for payment
